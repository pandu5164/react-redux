    ...
    <>
      <h1 style={{ margin: 10 }}>
        ...
        {counterState.count > 0 ? (
          ...
          <span style={{ color: "red" }}>Please click on add button first</span>
        )}
      </h1>
      <br />
      ...
      {counterState.count > 0 && (
        <div>
          <button style={{ margin: 10 }} onClick={makeApiCall}>
            Make api call
          </button>
        </div>
      )}
      ...
      <br />
      {counterState.data.length > 0 && (
        <Link
          style={{ margin: 10 }}
          to={`/display/${counterState.count}`}
          relative="path"
        >
          Navigate to api response page
        </Link>
      )}
    </>
  );
